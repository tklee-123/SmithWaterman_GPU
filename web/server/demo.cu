...
// }
std::string io_seq(const std::vector<char> &seqA_aligned, const std::vector<char> &seqB_aligned) {
  std::string result;

  int align_len = seqA_aligned.size();
  for (int i = 0; i < align_len + 1; ++i) {
    result += seqA_aligned[align_len - i];
  }
  return result;
}
...
void process_and_compare_line(const std::string& line, std::vector<char>& seqA, std::vector<std::tuple<int, std::string,std::string>>& scores) {
    ...
    if (parts.size() == 2) {
        ...
        std::vector<char> seqB_aligned;
        std::string sub;
        traceback(dir_cpu, max_id, arrA, arrB, seqA_aligned, seqB_aligned);
        sub = io_seq(seqA_aligned, seqB_aligned);
        std::tuple<int, std::string, std::string> element(score, parts[1],sub);
...
void read_and_compare_sequences_from_file(const std::string& filename, std::vector<char>& seqA, std::vector<std::tuple<int, std::string,std::string>>& scores, int num_threads) {
  std::ifstream file(filename);
  if (!file.is_open()) {
...
int main(int argc, char* argv[]) {
  ...
  int length = countLines("data.txt");
  ...
  read_and_compare_sequences_from_file("data.txt", seqA, scores, num_threads);
  std::cout << "Số điểm tương đồng được tính:" << scores.size() << std::endl;
  auto end_time = std::chrono::high_resolution_clock::now();

  // Sort the scores list
  std::sort(scores.begin(), scores.end(), [](const auto& a, const auto& b) {
    return std::get<0>(a) < std::get<0>(b);  // Compare based on the first element of the tuple (integer value)
});
  ...
  for (const auto& score : scores) {
    if (std::get<0>(score) > maxScore){
        maxScore = std::get<0>(score);
        label = std::get<1>(score);
        sub = std::get<2>(score);
    }
    }
